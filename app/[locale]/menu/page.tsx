          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="relative h-72 rounded-3xl overflow-hidden shadow-lg border border-[var(--color-crema-dark)] hover:shadow-xl transition-shadow">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-0 inset-x-0 p-6 text-white text-center">
                  <h3 className="font-display text-3xl font-bold">{t('pastaFumoTitle')}</h3>
                  <p className="font-bold mt-1 text-sm invisible" aria-hidden="true">{t('pastaAglioneNote')}</p>
                  <div className="text-5xl font-bold text-green-300 mt-2">{t('pastaFumoPrice')}</div>
                </div>
              </div>
              <div className="relative h-72 rounded-3xl overflow-hidden shadow-lg border border-[var(--color-crema-dark)] hover:shadow-xl transition-shadow">
                <Image
                  src="/pasta_aglione.jpg"
                  alt={t('pastaAglioneTitle')}
                  fill
                  className="object-cover"
                  sizes="(max-width: 768px) 100vw, 50vw"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-0 inset-x-0 p-6 text-white text-center">
                  <h3 className="font-display text-3xl font-bold">{t('pastaAglioneTitle')}</h3>
                  <p className="text-yellow-400 font-bold mt-1 text-sm">{t('pastaAglioneNote')}</p>
                  <div className="text-5xl font-bold text-green-300 mt-2">{t('pastaAglionePrice')}</div>
                </div>
              </div>
            </div>
          </div>
